...
<form>
	<Label class="mt-4 mb-2">Files for context</Label>
	<Select.Root bind:value={selectedFiles} type="multiple" name="files">
		...
		<Select.Content>
			{#each files as file (file.id)}
				<Select.Item value={file.utURL}>{file.name}</Select.Item>
			{/each}
		</Select.Content>
	</Select.Root>
	<Label class="mt-4 mb-2">Select interrogation date</Label>
	<Calendar
		bind:value={date}
		type="single"
	...
	/>
	<input bind:value={dateToString} type="hidden" />
</form>
